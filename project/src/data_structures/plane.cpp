...
Mat plane::getABase() {
	Mat P(3, 3, CV_64FC1);
	Vec3d direction1 = this->getDirection();
	Vec3d direction2 = Vec3d(direction1[2], 0, -direction1[0]);
	Vec3d direction3 = Vec3d(direction1[0] * direction1[1], - direction1[0] * direction1[0] - direction1[2] * direction1[2], direction1[2]);
	direction1 = direction1 / norm(direction1);
	direction2 = direction2 / norm(direction2);
	direction3 = direction3 / norm(direction3);
	P.at<double>(0, 0) = direction1[0];
	P.at<double>(0, 1) = direction1[1];
	P.at<double>(0, 2) = direction1[2];
	P.at<double>(1, 0) = direction2[0];
	P.at<double>(1, 1) = direction2[1];
	P.at<double>(1, 2) = direction2[2];
	P.at<double>(2, 0) = direction3[0];
	P.at<double>(2, 1) = direction3[1];
	P.at<double>(2, 2) = direction3[2];
	return P;
};
